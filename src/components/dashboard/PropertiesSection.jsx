...
export default function PropertiesSection() {
  const [properties, setProperties] = useState([
    ...
    },
    {
      ...
      bookings: 6,
      earnings: 12000,
      image: 'https://images.unsplash.com/photo-1503951458645-643d53bfd90f?q=80&w=2070&auto=format&fit=crop',
      occupancyRate: 45,
      averageRating: 4.3
    },
    {
  ...
  const getFilteredAndSortedProperties = () => {
    ...
    // Apply status filter
    if (statusFilter !== 'all') {
      filteredProperties = properties.filter(property => property.status === statusFilter);
    }
          ...
          {getFilteredAndSortedProperties().map(property => (
            ...
            >
              <img 
                src={property.image} 
                alt={property.title}
                className={view === 'grid' ? 'w-full h-48 object-cover' : 'w-48 h-32 object-cover'}
              />
              <div className="p-4 flex-1">
                ...
                <div className="mt-2 flex items-center space-x-2">
                  <span className={`badge ${
                    property.status === 'active' ? 'badge-success' : 'badge-error'
                  }`}>
                    {property.status}
                  </span>
                  <span className="badge badge-accent">{property.bookings} bookings</span>
                </div>
                <p className="mt-2 text-primary font-semibold">
                  ${property.earnings.toLocaleString()}
                </p>
                <div className="mt-4 flex space-x-2">
                  <button className="btn btn-sm btn-ghost" onClick={() => toast.info('Edit form coming soon!')}>
                    <FaEdit />
                  </button>
                  <button className="btn btn-sm btn-ghost" onClick={() => toast.info('Preview coming soon!')}>
                    <FaEye />
                  </button>
                  <button 
                  ...
                  <button 
                    className={`btn btn-sm ${property.status === 'active' ? 'btn-error' : 'btn-success'}`}
